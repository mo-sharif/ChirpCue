...
- [x] Personal-experience questions without an About you brief receive an ordered, noninvented instant bridge. With a brief, ChirpCue can locally select up to two relevant, already-speakable user sentences verbatim for an immediate Quick answer; model Quick and Deep may use only user-supplied personal facts. Shared validation accepts natural first-person openings and two short declarative sentences while rejecting instruction-like brief text.
