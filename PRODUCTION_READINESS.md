...
- [x] Every eligible production turn immediately displays a deterministic question-aware local bridge, then starts generated Quick and high-reasoning Deep independently. Apple on-device or Codex Sol Low Quick may replace the bridge inside 15 seconds; Deep starts one second later and remains eligible for 90 seconds. Model-controlled `needsDeep` never suppresses Deep.
...
- [x] Personal-experience questions without an About you brief receive an ordered, noninvented instant bridge. With a brief, Quick and Deep may use only its user-supplied facts; shared validation accepts natural `I've` openings and two short declarative sentences.
