...
- [ ] The live Quick-to-Deep prompter handoff meets real-meeting latency targets. September 4 warmed-runtime synthetic probes through the real ChatGPT subscription, `LowLatencyMeetingResponseGenerator`, and `ResponseCoordinator` measured Sol Quick at 8.92 s and 8.01 s (failed the eight-second target). After preferring Spark and removing duplicated Quick instructions, three samples passed across mutex/semaphore and Bloom-filter questions: Quick at 5.94/6.99/6.64 s and Astra Medium Deep at 11.21/13.64/12.06 s (96/90/95 words). All verified overlapping dispatch, binding to the visible Quick cue, one generation per lane, and cleanup. These samples exclude audio/transcription and SwiftUI render latency; they are not latency guarantees.
- [ ] Consistent longer personal-example handoffs need further live validation. A September 4 synthetic profile probe with the expanded answer budget passed at 7.28 s for Spark Quick and 15.22 s for a 136-word Astra Medium Deep answer using supplied feature-flag and pilot-group facts. Actual provider dispatches were 0.17 s and 1.01 s, so both lanes overlapped; binding and final cleanup passed. The preceding attempt reported Quick cleanup unavailable and reached Deep without a generated Quick. That intermittent failure was not reproduced or diagnosed by the passing rerun and remains an open reliability risk. The live harness now reports lane-specific, content-free failure stages.
...
- [x] The live-coaching routing policy prefers Spark for Quick, falling back to Sol, Luna, and mini when absent or hidden. Quick requests no reasoning only when advertised, otherwise the nearest supported effort, plus the fastest advertised subscription tier. The selected effort reaches the real Quick turn instead of being hardcoded to Low. Both Deep workloads prefer Astra Medium, falling back to Sol then Terra when missing, hidden, or reasoning-incompatible. Unknown models remain ineligible, and the earlier versioned policy remains available for pinned probes.
- [x] September 4 Astra subscription probe: ChirpCue's signed-in profile selected `gpt-6-astra` with `medium` effort and returned a validated 89-word general answer in 12.4 seconds. The complete probe, including cleanup, passed in 16.5 seconds. This is one measured sample, not a latency guarantee.
- [x] General Deep supports up to 220 words, 3,000 UTF-8 bytes, and twelve spoken sentences, with prompts targeting 120–180 words including an example and tradeoff. A matching supplied personal story takes priority; otherwise examples must be explicitly hypothetical. Quick retains its 24-word opening; verified repository answers retain exact extractive checks. The prompter adds paragraph breaks without changing the stored answer.
- [x] The explicit, locally saved speaker profile accepts up to 8,000 characters. Quick receives a 1,500-character prefix while Deep can use the fuller profile under provider-specific payload caps. Deep snapshots up to 180 seconds and 32 transcript segments; Codex prompt text is further capped at 12,000 characters. Quick retains its 45-second context. Tests verify older context is excluded from Quick and included in Deep, legacy turns work without the optional wider context, and Stop clears transcript state. Existing 180-second ephemeral retention is unchanged; ChatGPT memories and Codex chat history are not imported.
- [x] September 4 local subscription probe: a repository-free multipart comparison produced a validated 91-word Deep answer; generation, validation, and verified cleanup completed in 16.3 seconds. A separate run completed in 12.6 seconds. These bounded probes do not establish a latency guarantee or complete the live-meeting gates below.
- [x] Conversational question lead-ins are recognized without punctuation. Specific safe clarification and abstention text is preserved. Busy Deep admission waits at most 15 seconds without cancelling earlier answers, and provider Quick failures retain their actual failure category. Subscription capacity and local rate-limit failures are not automatically retried.
- [x] Every eligible production turn immediately displays a question-aware local cue, including during provider preparation, then runs generated Quick and Deep independently. Matching About you facts and reviewed technical answers remain instant. Codex skips the three-second Apple generation attempt and dispatches subscription Quick directly for uncovered questions. Other providers retain the bounded Apple attempt. An identical result is suppressed. Deep starts immediately after a local Quick succeeds, or gives provider Quick a one-second queue head start. Quick has a 15-second deadline and Deep 90 seconds; model-controlled needsDeep never suppresses Deep.
...
- [x] Clearly attributed local speech does not cancel generated work or hide an additive Deep answer: validated Deep displays next to the already-visible Quick before the transcript becomes final. A queued replacement of the spoken cue still waits for the speech boundary, with any Deep bound to that replacement released after it. Controller tests cover unchanged Quick content, Deep-ready timing before speech ends, dismissal, ordered pending upgrades, and echo reattribution.
...
- [x] A validated Quick result can display before its tracked ephemeral-fork cleanup finishes. Pending and cancellation cleanup is coalesced and joined before reopening or shutdown; rejected Quick, Deep, reconciliation, cancellation, missing receipt, delete, and journal-update failures block later inference until one bounded provider replacement reconciles every journaled thread. A replacement that cannot validate or finish cleanup remains fail-closed without retrying indefinitely.
...
- [x] Transport failure and confirmed response-cleanup failure trigger one coalesced recovery boundary: the failed client and active work are joined, journaled thread cleanup is verified, and Quick and Deep bases rebuild atomically before inference reopens. Detailed work interrupted by that replacement retries once against the recovered provider; Quick keeps its local cue. An unresolved cleanup failure remains visible, and neither a same-turn Deep result nor a deterministic bridge can hide it.
