...
- [x] Version `0.3.11` documents the reviewed direct technical primers, whole-word classification, and stable mid-speech Quick/Deep upgrades without changing any live, manual, or distribution gate.
...
- [x] Every eligible production turn immediately displays a deterministic question-aware local bridge or matching user-written fact, including while the selected provider runtime is still preparing, then starts generated Quick and high-reasoning Deep independently. Apple on-device or Codex Sol Low Quick may replace the bridge inside 15 seconds; an identical generated Quick is suppressed so the first answer does not flicker. Deep starts without the one-second provider head start after a local Quick succeeds; the head start remains only when provider Quick is needed. Deep remains eligible for 90 seconds. Model-controlled `needsDeep` never suppresses Deep.
...
- [x] A 17-question frontend and systems matrix requires reviewed direct primers to remain policy-safe, at most 24 words, and complete in under 100 milliseconds as a batch. Whole-word and phrase matching prevents substring category collisions, while everyday uses of promise, hydration, and hooks fail back safely. A coordinator regression proves a technical primer is the first event without waiting for either model.
...
- [x] The opt-in Apple on-device live smoke produced a complete validated Quick in 7.52 seconds and first streamed model text in 3.84 seconds on the development Mac. Production still displays only the completed locally validated result; streamed timing is diagnostic evidence, not early UI output.
