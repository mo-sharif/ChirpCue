...
PaceNote is a consent-first personal macOS meeting coach. It captures the microphone and Mac system output into separate local transcript lanes, shows a short sentence to say immediately, and then follows with a deeper answer grounded in one explicitly selected repository and optional read-only skill.
...
4. Select the meeting output source, choose the capture lanes, and select one repository. A repository-local domain skill is optional.
...
During a meeting, every eligible turn immediately shows the fixed local **SAY NOW** bridge “Let me dig into the exact implementation for a second.” The production coordinator never runs or displays a model-written Quick answer. Deep starts automatically. A technical answer appears only after exact extractive evidence verification: it must match one verified basis claim while preserving punctuation, and that claim must copy one complete cited source line. For a Deep clarification or abstention result, PaceNote ignores model prose and shows fixed local safe text. If validation fails, no Deep card appears; the bridge remains visible with a limited-mode status. No classifier or model flag can bypass the bridge or suppress Deep. Speak a suggestion yourself only if it is accurate and appropriate.
...
- **Deep answer unavailable:** select a repository, resolve any approvable ambiguous sensitive-file review, and check the visible Codex, grounding-budget, or cleanup status. High-confidence credential matches cannot be approved. PaceNote fails closed instead of reading the live working tree or broadening permissions.
