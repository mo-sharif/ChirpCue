...
- **Quick-first scheduling:** reviewed answers avoid a model turn entirely. For uncovered questions, Apple gets a three-second improvement budget; one timeout opens a meeting-scoped circuit so later questions reach the provider immediately. Codex Quick then uses a compact one-field, tool-free response on a prewarmed ephemeral thread and keeps a one-second provider-queue head start over Deep. The experimental realtime endpoint remains disabled.
...
- **Factual personal answers:** an optional local **About you** brief gives Quick and Deep the exact years, recent work, and role details they need instead of guessing. ChirpCue can retrieve a relevant, already-speakable sentence locally as the first answer, before provider preparation finishes, without flashing a generic opener or repeating the same Quick text.
...
Version 0.3.12 makes 52 reviewed technical answers show as completed Quick cards immediately instead of looking like an AI response is still pending. Ambiguous security, MCP, and TypeScript wording now falls back to generated coaching instead of forcing an unrelated primer. A matching **About you** fact can still become the first visible answer while the selected subscription runtime is preparing, and Quick and Deep upgrades remain stable while you speak. The latest opt-in Apple smoke on this development Mac observed raw first text only after roughly 31 seconds. ChirpCue therefore gives the on-device model a three-second improvement budget; the production wrapper returned a safe result inside its four-second assertion, canceled and tracked the late work for cleanup, and bypasses Apple for later questions in the same meeting. Codex Quick then retains the rest of its 15-second window, uses GPT-5.6 Sol at low effort, requests the advertised subscription Fast tier, and returns only the spoken sentence. Deep remains eligible for 90 seconds. Every response thread is ephemeral and unsubscribed after use. The experimental Codex realtime endpoint stays disabled. The installed Codex 0.150 subscription runtime has still shown intermittent startup and turn stalls in opt-in live probes, so the immediate local answer is the hard real-time guarantee and subscription generation remains an explicit release gate. This remains an early personal release candidate.
