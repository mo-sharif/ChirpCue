...
final class PackagedMeetingCoachSkillTests: XCTestCase {
    func testPackagedSkillIsPresentExplicitOnlyAndIntegrityBound() throws {
        ...
        XCTAssertTrue(skill.contains("Answer directly, explain why"))
        XCTAssertTrue(skill.contains("120 to 180 words"))
        XCTAssertTrue(skill.contains("broadly applicable knowledge"))
        XCTAssertTrue(skill.contains("pragmatic staff engineer"))
        XCTAssertTrue(skill.contains("Write for the ear, not the page"))
        XCTAssertTrue(skill.contains("next spoken beat"))
        XCTAssertTrue(skill.contains("everyday words and natural contractions"))
