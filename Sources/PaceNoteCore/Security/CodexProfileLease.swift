...
public enum CodexProfileLeaseError: Error, Equatable, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        ...
        case .invalidProfileRoot, .invalidLockFile:
            "ChirpCue rejected an unsafe Codex profile lock path."
        case .alreadyInUse:
            "The dedicated ChirpCue Codex profile is already in use. Quit ChirpCue and any live probe before trying again."
        case .unavailable:
            "ChirpCue could not acquire exclusive access to its dedicated Codex profile."
        }
    }
}

/// Holds an advisory lock for the lifetime of one ChirpCue profile owner.
///
