...
source_app="$project_root/dist/PrismCue.app"
destination_app="/Applications/PrismCue.app"
staging_root=""
staging_app=""

case "$destination_app" in
    /Applications/PrismCue.app) ;;
    *) printf '%s\n' "Refusing unsafe application destination" >&2; exit 1 ;;
esac
if test -e "$destination_app" || test -L "$destination_app"; then
    printf '%s\n' "PrismCue is already installed. Remove the existing app deliberately before replacing it." >&2
    exit 1
fi

cleanup_failed_install() {
    case "$staging_root" in
        /Applications/.PrismCue.install.??????)
...
staging_root=$(/usr/bin/mktemp -d "/Applications/.PrismCue.install.XXXXXX")
case "$staging_root" in
    /Applications/.PrismCue.install.??????) ;;
    *) printf '%s\n' "Refusing unsafe staging directory" >&2; exit 1 ;;
esac
staging_app="$staging_root/PrismCue.app"
...
if test -e "$staging_app" || test -L "$staging_app"; then
    printf '%s\n' "PrismCue appeared at the install destination during installation; nothing was replaced." >&2
    exit 1
fi
