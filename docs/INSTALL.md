...
- **Quick stays on the instant opener:** reviewed technical answers and matching **About you** facts are already completed Quick answers. For uncovered questions, Apple Intelligence gets three seconds to improve the opener. If it is slow or fails once, ChirpCue bypasses it for later questions in that meeting and, with Codex selected, routes Quick to GPT-5.6 Sol at low effort with the advertised subscription Fast tier. If no generated answer finishes within 15 seconds, the opener stays visible while Deep continues for up to 90 seconds.
